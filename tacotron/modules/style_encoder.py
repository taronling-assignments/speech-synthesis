...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        K = len(hp.ref_encoding_filters)
        filters = [1] + hp.ref_encoding_filters
        ...
        self.bns = nn.ModuleList([nn.BatchNorm2d(num_features=hp.ref_encoding_filters[i]) for i in range(K)])

        out_channels = self.calculate_channels(hp.n_mels, 3, 2, 1, K)
        self.gru = nn.GRU(input_size=hp.ref_encoding_filters[-1] * out_channels,
                          hidden_size=hp.E // 2,
                          batch_first=True)
